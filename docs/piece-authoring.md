...
Where a Piece goes: the built-in Library is the `library/` folder of this repository, bundled into the app. The user's own Pieces are a `pieces` folder with the same layout, in the app's data folder or next to `BlockCreator.exe` as the user chooses (**Pieces → Custom Pieces**; **Open Folder** opens it), and win over built-in ones with the same `id`. A Piece for the built-in Library goes into `library/actions/` or `library/conditions/` and is tried with `npm run tauri dev`. To have an AI write a Piece, give it [piece-authoring-for-ai.md](piece-authoring-for-ai.md) as well.
