...
- **Mario-Seite ohne Überlappung**: Vanilla-Munchers verletzen seitlich nur bei echter Überlappung. Lokale Blöcke prüfen daher `$93`/`$94` gegen eine Tabelle `db $02,$0D` (`GPS/blocks/global/hurt_death.asm:47-58`, gleiches Muster in `teleport.asm`, `set_to_on.asm`). `$93` = Seite des Blocks, auf der Mario ist: **0 = Mario links vom Block, 1 = rechts** (korrigiert 2026-09-18: `DISASM/bank_00.asm:12054-12067` `CODE_00EB77` setzt `$93` = 0, wenn `(Mario-X & $0F) < 8` – bei 16 px Breite berührt er den Block dann von links; `hurt_death.asm` `SidePixelTable: db $02,$0D` erwartet für `$93` = 0 genau diesen bündigen Kontakt. Die frühere Angabe „0 rechts, 1 links“ aus `CLI memory 7E0093` beschreibt Marios Lage in seiner eigenen Kachel, nicht die Blockseite; **im Emulator noch zu bestätigen**, Ticket 19). Das Archiv kritisiert Blockreator genau dafür (`ARCH/block_41222_Black_Piranha_Plant_(Muncher)_v1.1/VanillaMuncher.asm:4-7`).
