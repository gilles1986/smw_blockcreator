...
- Piece authoring contract: a Condition jumps to `{{false}}` when false and falls through when true; the generator wires AND/OR/NOT and long branches. The manifest's `clobbers` lists registers the Piece destroys and the generator saves/restores them (X = sprite index, Y = act-as high byte). Local labels via `{{label "name"}}`, made unique by the generator. Code must be SA-1 compatible (`|!addr`, `!sprite_*` defines).
